...
public class BlockDesignInventory extends CustomScrollingInventory {
    ...
    public BlockDesignInventory(CookieClicker plugin, Player player, String title, int size, Integer[] skippedSlots, int startSlot, int stopSlot, CustomInventoryType customInventoryType) {
    ...
    public void update() {
        ...
        for (Map.Entry<Integer, Long> priceEntry : clickerStatsProfile.getPrices(shopType).entrySet()) {
            ...
            String typeName = pluginConfig.getTypeName(shopType, id);
            if (typeName.equalsIgnoreCase("PLAYER_HEAD")) {
                textureURL = pluginConfig.getSkullTexture(shopType, id);
            } else if (typeName.equalsIgnoreCase("REAL_PLAYER_HEAD")) {
                playerProfile = plugin.getReflectionUtil().getRealPlayerProfile(player);
            }
            if (clickerStatsProfile.getItems(shopType).get(id)) {
                if (clickerStatsProfile.getBlockDesign() == id) {
                    items.add(itemBuilder.build(type, languageConfig.getPriceLanguage(PluginLanguage.ITEM_BOUGHT_SELECTED, shopType, id, price), 1, languageConfig.getPriceLanguages(PluginLanguage.ITEM_BOUGHT_SELECTED_INFO, shopType, id, price), true, true, textureURL, playerProfile));
                    continue;
                }
                items.add(itemBuilder.build(type, languageConfig.getPriceLanguage(PluginLanguage.ITEM_BOUGHT, shopType, id, price), 1, languageConfig.getPriceLanguages(PluginLanguage.ITEM_BOUGHT_INFO, shopType, id, price), false, true, textureURL, playerProfile));
                continue;
            }
            if (cookies >= price) {
                items.add(itemBuilder.build(type, languageConfig.getPriceLanguage(PluginLanguage.PRICE_BUYABLE, shopType, id, price), 1, languageConfig.getPriceLanguages(PluginLanguage.ITEM_INFO, shopType, id, price), false, true, textureURL, playerProfile));
                continue;
            }
            items.add(itemBuilder.build(type, languageConfig.getPriceLanguage(PluginLanguage.PRICE_NOT_BUYABLE, shopType, id, price), 1, languageConfig.getPriceLanguages(PluginLanguage.ITEM_INFO, shopType, id, price), false, true, textureURL, playerProfile));
        }
        plugin.getServer().getScheduler().runTaskLaterAsynchronously(plugin, () -> loadPage(), 2);
        if (hasNext()) {
