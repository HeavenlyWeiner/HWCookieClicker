...
public class MenuDesignInventory extends CustomScrollingInventory {
    ...
    public MenuDesignInventory(CookieClicker plugin, Player player, String title, int size, Integer[] skippedSlots, int startSlot, int stopSlot, CustomInventoryType customInventoryType) {
    ...
    public void update() {
        ...
        ShopType shopType = ShopType.MENU_DESIGN;
        ...
        for (Map.Entry<Integer, Long> priceEntry : clickerStatsProfile.getPrices(shopType).entrySet()) {
            ...
            if (clickerStatsProfile.getItems(shopType).get(id)) {
                if (clickerStatsProfile.getMenuDesign() == id) {
