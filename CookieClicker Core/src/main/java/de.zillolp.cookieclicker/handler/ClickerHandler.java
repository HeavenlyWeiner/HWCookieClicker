...
public class ClickerHandler {
    private final PlatformLockManager platformLockManager;
    ...
    public ClickerHandler(CookieClicker plugin) {
        ...
        this.platformLockManager = plugin.getPlatformLockManager();
    }
    ...
    public void handleClickerInteract(CookieClickerInteractEvent event) {
        bukkitScheduler.runTaskAsynchronously(plugin, () -> {
            ...
            platformLockManager.canClick(uuid).thenAccept(lockResult -> {
                if (!lockResult.canClick()) {
                    // Игрок заблокирован - кликает в другой платформе
                    if (lockResult.isBlockedByTelegram()) {
                        bukkitScheduler.runTask(plugin, () -> {
                            // Отправить сообщение из language.yml
                            player.sendMessage(languageConfig.getReplacedLanguage(PluginLanguage.PLATFORM_LOCKED_MINECRAFT, uuid));
                            soundManager.playSound(player, SoundType.CLICK_DENY);
                        });
                    }
                    return;  // Прервать обработку клика
                }

                // ВСЁ ЧТО НИЖЕ - выполняется только если НЕ заблокирован
