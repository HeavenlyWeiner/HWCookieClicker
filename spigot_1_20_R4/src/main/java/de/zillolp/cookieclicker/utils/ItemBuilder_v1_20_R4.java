...
public class ItemBuilder_v1_20_R4 implements ItemBuilder {
    private final Logger logger;
    private final ReflectionUtil reflectionUtil;

    public ItemBuilder_v1_20_R4(CookieClicker plugin) {
    ...
    public ItemStack build(Material material, String displayName, int amount, String[] lore, boolean hasEnchantment, boolean hideItemInfos, String textureURL, PlayerProfile playerProfile) {
        ...
        if (hideItemInfos) {
            if (!(reflectionUtil.isSpigot())) {
                AttributeModifier attributeModifier = new AttributeModifier("cookieclicker_hide_attribute", 0.0, AttributeModifier.Operation.ADD_NUMBER);
